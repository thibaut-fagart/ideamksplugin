...
public class ListChangePackageEntries extends SiCLICommand {
    ...
    public ListChangePackageEntries(List<VcsException> errors, MksCLIConfiguration mksCLIConfiguration, MksChangePackage mksChangePackage) {
...
//				System.out.println("ListChangePackageEntries " + Arrays.asList(parts));
    ...
    protected Date parseDate(String date) throws VcsException {
        try {
            return format.parse(date);
        } catch (ParseException e) {
            throw new VcsException("unknown date forma for " + date + " (expected [" + "MMM dd, yyyy - hh:mm:ss a" + "])");
        }
    }
}
