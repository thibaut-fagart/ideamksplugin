...
public class ViewSandboxLocalChangesOrLockedCommandTest extends TestCase {
	...
	public void testFindsModifiedWithoutCheckout() {
		...
		assertTrue("bad modifiedWithoutCheckout status",
				memberState.status == MksMemberState.Status.MODIFIED_WITHOUT_CHECKOUT);
	...
	public void testViewsCheckedOutInAnotherSandboxBySameUserAsUnknown() {
		...
		commandLocal = createCommand(errors, sandboxPath, "viewsandbox/changedOrLocked.txt");
		commandLocal.execute();
		String checkedOutInAnotherSandboxFile =
				SANDBOX_DIR + "\\HJE-Batch\\src\\com\\hsbc\\hbfr\\ccf\\at\\batch\\Batch.java";
	...
	private ViewSandboxLocalChangesOrLockedCommand createCommand(final List<VcsException> errors,
																 final String sandboxPath, final String outputFile) {
