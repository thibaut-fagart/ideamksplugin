...
public class SandboxCacheImpl implements SandboxCache {
    ...
    private final HashSet<VirtualFile> sandboxVFiles =
        new HashSet<VirtualFile>();
    private final HashSet<MksSandboxInfo> outOfScopeSandboxes =
        new HashSet<MksSandboxInfo>();
    @NotNull
     ...
     * intialized
     */
    private final ArrayList<MksSandboxInfo> pendingUpdates =
        new ArrayList<MksSandboxInfo>();
        ...
        public void run() {
            while (!stopBackgroundThread) {
                ...
                synchronized (pendingUpdates) {
                    tempList =
                        new ArrayList<MksSandboxInfo>(pendingUpdates.size());
    ...
    public boolean isSandboxProject(@NotNull VirtualFile virtualFile) {
        return sandboxVFiles.contains(virtualFile) || virtualFile.getName()
            .equals("project.pj");
    }

    public void addSandboxPath(@NotNull String sandboxPath,
                               @NotNull final String mksHostAndPort,
                               @NotNull String mksProject,
                               @Nullable String devPath, boolean isSubSandbox) {

        VirtualFile sandboxVFile = VcsUtil.getVirtualFile(sandboxPath);
        MksSandboxInfo sandboxInfo = new MksSandboxInfo(sandboxPath,
            mksHostAndPort, mksProject, devPath, sandboxVFile, isSubSandbox);
    ...
    private void addSandbox(MksSandboxInfo sandboxInfo) {
        if (doesSandboxIntersectProject(new File(sandboxInfo.sandboxPath))) {
            ...
            LOGGER.debug("ignoring out of project sandbox [" + sandboxInfo
                .sandboxPath + "]");
        }
    }
     ...
     */
    private void addSandboxBelongingToProject(
        @NotNull MksSandboxInfo sandboxInfo) {
        String sandboxPath;
        VirtualFile sandboxVFile;
        sandboxPath = sandboxInfo.sandboxPath;
        sandboxVFile = sandboxInfo.sandboxPjFile == null ?
            VcsUtil.getVirtualFile(sandboxInfo.sandboxPath) :
            sandboxInfo.sandboxPjFile;
        VirtualFile sandboxFolder;
        if (sandboxVFile != null) {
            sandboxFolder = (sandboxVFile.isDirectory()) ? sandboxVFile :
                sandboxVFile.getParent();
            if (sandboxFolder == null) {
                LOGGER.warn("unable to find parent VirtualFile for sandbox ["
                    + sandboxVFile + "]");
            }
...
//				try {
                List<MksSandboxInfo> infoList =
                    sandboxByFolder.get(sandboxFolder);
                if (infoList == null) {
                ...
                if (!sandboxInfo.isSubSandbox) {
                    final VirtualFile sandboxParentFolderVFile =
                        sandboxVFile.getParent();
                    LOGGER.info(
                        "marking " + sandboxParentFolderVFile + "as dirty");
...
//				}
            ...
            if (sandboxPjFile != null) {
                sandboxInfo = new MksSandboxInfo(sandboxInfo.sandboxPath,
                    sandboxInfo.hostAndPort, sandboxInfo.mksProject,
                    sandboxInfo.devPath, sandboxPjFile,
                    sandboxInfo.isSubSandbox);
            }
            addRejected(sandboxInfo);
        }
    }
    ...
    private boolean doesSandboxIntersectProject(@NotNull File sandboxFile) {
        ...
        boolean sandboxRelevant =
            (sandboxVFile != null) && projectScope.contains(sandboxVFile);
        if (!sandboxRelevant) {
            VirtualFile[] projectContentRoots =
                ProjectRootManager.getInstance(project).getContentRoots();
            for (int i = 0; i < projectContentRoots.length && !sandboxRelevant;
                 i++) {
                VirtualFile projectContentRoot = projectContentRoots[i];
                if (VfsUtil.isAncestor(sandboxFolder,
                    VfsUtil.virtualToIoFile(projectContentRoot), true)) {
                    LOGGER.debug("sandbox [" + sandboxFolder
                        + "] contains contentRoot [" + projectContentRoot
                        + "]");
    ...
    private void addRejected(final MksSandboxInfo sandbox) {
        ...
        if (sandbox.retries > MAX_RETRY) {
            LOGGER.debug("giving up sandbox after too many retries " + sandbox
                .sandboxPath);
        }
        synchronized (pendingUpdates) {
    ...
    public void dumpStateOn(@NotNull PrintWriter pw) {
        ...
        List<MksSandboxInfo> sortList =
            new ArrayList<MksSandboxInfo>(sandboxByFolder.size());
        ...
        Comparator<MksSandboxInfo> comparator =
            new Comparator<MksSandboxInfo>() {
                public int compare(final MksSandboxInfo first,
                                   final MksSandboxInfo other) {
                    if (first == null) {
                        return -1;
                    } else if (other == null) {
                        return 1;
    ...
    public void rootsChanged(final ModuleRootEvent event) {
        ...
        synchronized (lock) {
            List<MksSandboxInfo> allSandboxes = new ArrayList<MksSandboxInfo>(
                sandboxVFiles.size() + outOfScopeSandboxes.size());
            for (List<MksSandboxInfo> infoList : sandboxByFolder.values()) {
     ...
     */
    @NotNull
    public Set<MksSandboxInfo> getSandboxesIntersecting(
        ...
        synchronized (lock) {
            sandboxInfoListOfList =
                new ArrayList<List<MksSandboxInfo>>(sandboxByFolder.values());
        }
        for (List<MksSandboxInfo> infoList : sandboxInfoListOfList) {
            for (MksSandboxInfo sandboxInfo : infoList) {
                ...
                if (sandboxFile == null) {
                    synchronized (lock) {
                        LOGGER.warn(
                            "SandboxInfo with NULL virtualFile !! removing from registered sandboxes");
                    ...
                    final VirtualFile sandboxParentFile =
                        sandboxFile.getParent();
                    if (sandboxParentFile != null && VfsUtil
                        .isAncestor(sandboxParentFile, directory, false)) {
                        result.add(sandboxInfo);
                    }
                }
            }
...
*/
     ...
     */
    @Nullable
    public MksSandboxInfo getSandboxInfo(
    ...
    private MksSandboxInfo getSandbox(VirtualFile virtualFile,
        ...
        VirtualFile cursorDir =
            (virtualFile.isDirectory() ? virtualFile : virtualFile.getParent());
        MksSandboxInfo foundSubSandbox = null;
        for (; cursorDir != null && sandbox == null;
             cursorDir = cursorDir.getParent()) {
     ...
     */
    private boolean checkSandboxContains(@NotNull MksSandboxInfo sandbox,
                                         @NotNull VirtualFile virtualFile) {
        final FilePath filePath = PeerFactory.getInstance()
            .getVcsContextFactory().createFilePathOn(virtualFile);
        if (!filePath.getIOFile().exists() || sandbox.sandboxPjFile == null) {
            return false;
        }
        final FilePath sandboxFolderFilePath = PeerFactory.getInstance()
            .getVcsContextFactory()
            .createFilePathOn(sandbox.sandboxPjFile.getParent());

        AbstractViewSandboxCommand command = new AbstractViewSandboxCommand(
            new ArrayList<VcsException>(), project.getComponent(MksVcs.class),
            sandbox.sandboxPath, "--filter=file:" + MKSHelper
            .getRelativePath(filePath, sandboxFolderFilePath)) {
            @Override
            protected MksMemberState createState(String workingRev,
                                                 String memberRev,
                                                 String workingCpid,
                                                 String locker,
                                                 String lockedSandbox,
                                                 String type, String deferred)
                throws VcsException {
                return new MksMemberState(
                    MksRevisionNumber.createRevision(workingRev),
        ...
        if (command.foundError()) {
            LOGGER.error("error while checking if sandbox " + sandbox
