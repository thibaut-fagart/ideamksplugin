...
class MKSChangeProvider extends AbstractProjectComponent
    ...
    private final Logger BUILDER_PROXY_LOGGER =
        Logger.getInstance(getClass().getName() + ".ChangelistBuilder");
    ...
    public void getChanges(final VcsDirtyScope dirtyScope,
        ...
        try {
            ...
            WindowManager.getInstance().getStatusBar(getProject())
                .addCustomIndicationComponent(statusLabel);
            Set<MksSandboxInfo> sandboxesToRefresh =
                collectSandboxesToRefresh(dirtyScope, statusLabel);
            ...
            checkNeededServersAreOnlineAndReconnectIfNeeded(sandboxesToRefresh,
                servers);
            setStatusInfo(statusLabel,
                MksBundle.message("collecting.change.packages"));
            // collect affected sandboxes
//			if (MksVcs.DEBUG) {
//				builder = createBuilderLoggingProxy(builder);
//			}
            final Map<String, MksServerInfo> serversByHostAndPort =
                distributeServersByHostAndPort(servers);
            ...
            for (Map<String, MksChangePackage> packageMap : changePackagesPerServer
                .values()) {
                for (Map.Entry<String, MksChangePackage> entry : packageMap
                    .entrySet()) {
                    final ChangeList changeList =
                        adapter.getChangeList(entry.getValue());
                    if (changeList != null && !entry.getValue().getSummary()
                        .equals(changeList.getName())) {
                        if (changeList instanceof LocalChangeList) {
                            ((LocalChangeList) changeList)
                                .setName(entry.getValue().getSummary());
                        }
                    }
                }
            }
            for (MksSandboxInfo sandbox : sandboxesToRefresh) {
                @Nullable MksServerInfo sandboxServer =
                    serversByHostAndPort.get(sandbox.hostAndPort);
                if (sandboxServer == null) {
                    logger.warn("sandbox [" + sandbox.sandboxPath
                        + "] bound to unknown or not connected server["
                        + sandbox.hostAndPort + "], skipping");
                    continue;
                }
                final String message = MksBundle.message(
                    "requesting.mks.sandbox.name.index.total",
                    sandbox.sandboxPath,
                    ++refreshedSandbox, sandboxCountToRefresh);
                setStatusInfo(statusLabel, message);
                Map<String, MksChangePackage> changePackageMap =
                    changePackagesPerServer.get(sandboxServer);
                if (changePackageMap == null) {
                    changePackageMap = Collections.emptyMap();
                }
                processDirtySandbox(builder, changePackageMap,
                    getSandboxState(sandbox, errors, sandboxServer));
            }
            final ChangelistBuilder finalBuilder = builder;
            // iterate over the local dirty scope to flag unversioned files
            setStatusInfo(statusLabel,
                MksBundle.message("processing.unversioned.files"));
                ...
                public boolean process(FilePath filePath) {
                    if (filePath.isDirectory()) {
                        ...
                        logger.warn("no VirtualFile for " + filePath.getPath()
                            + ", ignoring");
                        return true;
                    } else if (getMksvcs().getSandboxCache()
                        .isSandboxProject(filePath.getVirtualFile())) {
                        finalBuilder
                            .processIgnoredFile(filePath.getVirtualFile());
            ...
            WindowManager.getInstance().getStatusBar(getProject())
                .removeCustomIndicationComponent(statusLabel);
    ...
    private void checkNeededServersAreOnlineAndReconnectIfNeeded(
        @NotNull Set<MksSandboxInfo> sandboxesToRefresh,
        ...
        for (final String hostAndPort : serversNeedingReconnect) {
            ...
            try {
                SwingUtilities.invokeAndWait(new Runnable() {
                    public void run() {
                        ...
                        userAndPassword.user = Messages.showInputDialog(
                            getMksvcs().getProject(), "user", message, null);
                        userAndPassword.password = Messages.showInputDialog(
                            getMksvcs().getProject(), "password", message,
                            null);
            ...
            if (userAndPassword.user == null
                || userAndPassword.password == null) {
                continue;
            }
            SiConnectCommand command = new SiConnectCommand(getMksvcs(), host,
                port, userAndPassword.user, userAndPassword.password);
            command.execute();
            if (!command.foundError() && (command.getServer() != null)) {
                servers.add(command.getServer());
            } else {
                reportErrors(command.errors,
                    "unable to connect to " + hostAndPort);
            }
        }
    }

    @NotNull
    private Set<MksSandboxInfo> collectSandboxesToRefresh(
        VcsDirtyScope dirtyScope, JLabel statusLabel) {
        setStatusInfo(statusLabel,
            MksBundle.message("collecting.relevant.sandboxes"));
        ...
        for (FilePath dir : dirtyScope.getRecursivelyDirtyDirectories()) {
            ...
            if (vFile != null) {
                Set<MksSandboxInfo> sandboxes = getMksvcs().getSandboxCache()
                    .getSandboxesIntersecting(vFile);
        ...
        for (FilePath path : dirtyScope.getDirtyFiles()) {
            ...
            if (vFile != null) {
                final MksSandboxInfo mksSandboxInfo =
                    getMksvcs().getSandboxCache().getSandboxInfo(vFile);
                if (mksSandboxInfo != null) {
                    getMksvcs().debug(
                        "=> dirtySandbox : " + mksSandboxInfo.sandboxPath);
    ...
    private Map<String, MksServerInfo> distributeServersByHostAndPort(
        ...
        Map<String, MksServerInfo> result =
            new HashMap<String, MksServerInfo>();
        for (MksServerInfo server : servers) {
    ...
    private void processDirtySandbox(final ChangelistBuilder builder, @NotNull
    final Map<String, MksChangePackage> changePackages,
                                     final Map<String, MksMemberState> states) {
        final ChangeListManager listManager =
            ChangeListManager.getInstance(getProject());
        for (Map.Entry<String, MksMemberState> entry : states.entrySet()) {
            ...
            if (null == virtualFile || getMksvcs().getSandboxCache()
                .isSandboxProject(virtualFile)) {
                continue;
            }
            ...
            switch (state.status) {
                case ADDED: {
                    MksChangePackage changePackage =
                        getChangePackage(changePackages, state);
                    ...
                    if (changePackage == null) {
                        ...
                        ChangeList changeList = getMksvcs()
                            .getChangeListAdapter()
                            .trackMksChangePackage(changePackage);
                ...
                case CHECKED_OUT: {
                    MksChangePackage changePackage =
                        getChangePackage(changePackages, state);
                    Change change = new Change(
                        new MksContentRevision(getMksvcs(), filePath,
    ...
    private MksChangePackage getChangePackage(
        ...
        return state.workingChangePackageId == null ? null :
            changePackages.get(state.workingChangePackageId);
    }
    ...
    private Map<String, MksMemberState> getSandboxState(
        ...
        Map<String, MksMemberState> states =
            new HashMap<String, MksMemberState>();

        ViewSandboxWithoutChangesCommand fullSandboxCommand =
            new ViewSandboxWithoutChangesCommand(errors, getMksvcs(),
                sandbox.sandboxPath);
        ...
        ViewSandboxLocalChangesOrLockedCommand localChangesCommand =
            new ViewSandboxLocalChangesOrLockedCommand(errors, getMksvcs(),
                server.user, sandbox.sandboxPath);
        localChangesCommand.execute();
        addNonExcludedStates(states, localChangesCommand.getMemberStates());

        ViewNonMembersCommand nonMembersCommand =
...
// todo the below belong to incoming changes
        ViewSandboxOutOfSyncCommand outOfSyncCommand =
            new ViewSandboxOutOfSyncCommand(errors, getMksvcs(),
                sandbox.sandboxPath);
...
//		states.putAll(missingCommand.getMemberStates());
    ...
    private void addNonExcludedStates(Map<String, MksMemberState> collectingMap,
                                      Map<String, MksMemberState> source) {
        for (Map.Entry<String, MksMemberState> entry : source.entrySet()) {
            final FilePath path = VcsUtil.getFilePath(entry.getKey());
            if (path.getVirtualFile() != null && myProject.getProjectScope()
                .contains(path.getVirtualFile())) {
    ...
    private Map<MksServerInfo, Map<String, MksChangePackage>> getChangePackages(
        final ProgressIndicator progress, final ArrayList<VcsException> errors,
        final ArrayList<MksServerInfo> servers) {
        final Map<MksServerInfo, Map<String, MksChangePackage>> changePackages =
            new HashMap<MksServerInfo, Map<String, MksChangePackage>>();
        final MksConfiguration config = ApplicationManager.getApplication()
            .getComponent(MksConfiguration.class);
        for (MksServerInfo server : servers) {
            if (!config.isServerSiServer(server)) {
                getMksvcs().debug("ignoring " + server.host + ":" + server.port
                    + " when querying changepackages");
                continue;
            }
            final ListChangePackages listCpsAction =
                new ListChangePackages(errors, getMksvcs(), server);
            if (progress != null) {
                progress.setIndeterminate(true);
                progress
                    .setText("Querying change packages for " + server + "...");
            }
            listCpsAction.execute();
            if (listCpsAction.foundError()) {
                reportErrors(listCpsAction.errors, "error querying mks cps");
            }
            config.serverIsSiServer(listCpsAction.serverInfo,
                listCpsAction.serverInfo.isSIServer);

            Map<String, MksChangePackage> serverChangePackages =
                new HashMap<String, MksChangePackage>();
    ...
    private ArrayList<MksServerInfo> getMksServers(
        ...
        final ListServers listServersAction =
            new ListServers(errors, getMksvcs());
        if (progress != null) {
            progress.setIndeterminate(true);
        ...
        if (listServersAction.foundError()) {
            reportErrors(listServersAction.errors,
                "encountered errors querying servers");
        }
    ...
    public void decorateChangeList(LocalChangeList changeList,
        ...
        if (!changeListAdapter
            .isChangeListMksControlled(changeList.getName())) {
            return;
        }
        MksChangePackage aPackage =
            changeListAdapter.getMksChangePackage(changeList.getName());
        if (aPackage != null) {

            cellRenderer.append(" - MKS #" + aPackage.getId(),
    ...
    private MksVcs getMksvcs() {
        return (MksVcs) ProjectLevelVcsManager.getInstance(myProject)
            .findVcsByName(MksVcs.VCS_NAME);
    }
}