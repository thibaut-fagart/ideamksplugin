    ...
    public void updateSandboxPath(@NotNull String sandboxPath, @NotNull String serverHostAndPort, @NotNull String mksProject, @Nullable String devPath, boolean isSubSandbox) {
        if (doesSandboxIntersectProject(new File(sandboxPath))) {
            ...
            synchronized (lock) {
                if (!sandboxVFiles.contains(sandboxVFile)) {
                    ...
                    for (MksSandboxInfo sandboxInfo : infoList) {
                        if (sandboxInfo.sandboxPath.equals(sandboxPath)) {
                            infoList.remove(sandboxInfo);
                            infoList.add(new MksSandboxInfo(sandboxPath, serverHostAndPort, mksProject, devPath, sandboxVFile, isSubSandbox));
            ...
            for (MksSandboxInfo sandbox : outOfScopeSandboxes) {
                if (sandboxPath.equals(sandbox.sandboxPath)) {
                    outOfScopeSandboxes.remove(sandbox);
                    outOfScopeSandboxes.add(new MksSandboxInfo(sandboxPath, serverHostAndPort, mksProject, devPath, null, isSubSandbox));
                    return;
                }
            }
        }
